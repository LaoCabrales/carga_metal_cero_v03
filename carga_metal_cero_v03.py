# -*- coding: utf-8 -*-
"""carga_metal_cero_v03.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1lr42AneV9Da_tnhk5uJ1F9aflDAFvV83
"""
...
# Establecer el locale a español para que strftime use nombres de días y meses en español
try:
    ...
    try:
        ...
        # Si las opciones específicas fallan, se mantiene el locale por defecto (ej. 'en_US')
        pass
...
# Inyectar CSS para modificar el color del texto de st.caption (subtítulos)
# Streamlit usa la clase .st-emotion-cache-1gcsx22 para los subtítulos (depende de la versión)
# Pero usaremos una solución más genérica si es posible, o marcadores directos.

CAPTION_STYLE = """
<style>
/* Estilo para los subtítulos de las listas de tareas (Sección 1) */
.stCaption:nth-child(2) > div,
/* Estilo para el subtítulo del calendario (Sección 2) */
.stCaption:nth-child(5) > div {
    color: #000000 !important; /* Forzar color negro */
    font-weight: 500;
}

/* Solución alternativa/más robusta para subtítulos generales en caso de que la primera falle: */
...
# Se utiliza el estado de sesión de Streamlit para almacenar las tareas de forma persistente
if 'tasks' not in st.session_state:
    st.session_state.tasks = []

# --- CONSTANTES DE CATEGORÍA Y ESTILOS ---
CATEGORIES = {
    "🔴 Urgente": {"color": "#FF4B4B", "label": "Urgente"},      # Rojo
    "🟠 Importante": {"color": "#FFA500", "label": "Importante"},  # Naranja
    "🟡 Pendiente": {"color": "#FFD700", "label": "Pendiente"}   # Amarillo/Dorado
}

# --- FUNCIONES AUXILIARES ---

def add_task(description, category_key, due_date):
    """Agrega una nueva tarea a la lista de tareas en el estado de sesión."""
    if description:
        # Extraer el nombre de la categoría (sin el emoji)
        category_name = CATEGORIES[category_key]["label"]

        # Crear la estructura de la nueva tarea
        new_task = {
            "id": str(uuid.uuid4()), # ID único para el manejo de tareas
...
def remove_task(task_id):
    ...
    st.experimental_rerun() # Forzar la actualización para reflejar el cambio inmediatamente
...
def move_task_to_calendar_callback(task_id):
    ...
    # Obtener la fecha seleccionada directamente del estado de sesión
    new_date = st.session_state.get(date_key)

    if new_date:
        if update_task_date(task_id, new_date):
            # Forzar la actualización inmediata de la UI para reflejar el cambio
            st.experimental_rerun()
    # No se maneja el error aquí, ya que el control de fecha lo hace st.date_input
...
def get_tasks_by_status():
    ...
    # Inicializar el diccionario con los nombres de las categorías como claves.
    ...
    # Ordenar las tareas con fecha cronológicamente
...
with st.sidebar:
    st.title("➕ Organiza tu Carga Mental")
    st.markdown("---")
    ...
    # 3. Fecha específica (Opcional)
    has_date = st.checkbox("¿Tiene fecha específica?", key="has_date_checkbox")
    due_date = None
    if has_date:
    ...
    st.markdown("---")
    st.markdown("🤖 **Objetivo:** Disminuir tu carga mental al visualizar y clasificar tus compromisos.")
...
st.title("📅 Tablero de Tareas Diarias")

# --- SECCIÓN 1: TAREAS POR ASIGNAR (LISTAS POR CATEGORÍA) ---
...
def display_task_list(column, category_label, tasks):
    """Muestra una lista de tareas en una columna con el estilo de su categoría."""
    ...
    with column:
        ...
        for task in tasks:
            ...
            with col_btn_del:
                if st.button("🗑️", key=f"del_no_date_{task['id']}", help="Eliminar tarea"):
                    remove_task(task['id'])

            with col_popover:
                # Botón para asignar fecha y mover al calendario
                with st.popover("📅 Asignar Fecha", use_container_width=True):
                    # El date_input establece su valor en st.session_state[key]
                    st.date_input(
                        "Selecciona una fecha para esta tarea:",
                        min_value=date.today(),
                        key=f"popover_date_{task['id']}" # La clave que usará el callback
                    )

                    # El botón usa on_click para llamar a la función que leerá el valor de la fecha
                    st.button(
                        "Mover a Calendario",
...
# --- SECCIÓN 2: CALENDARIO (TAREAS ASIGNADAS) ---

st.header("🗓️ Tareas Asignadas a Fecha (Calendario)")
# Se incluye el subtítulo dinámico con el color de letra forzado a negro
st.caption(f"Tienes **{len(tasks_with_date)}** tareas programadas.")

if not tasks_with_date:
    st.info("¡No tienes tareas con fecha específica aún! Agrégalas o asígnales una fecha desde las listas.")
else:
    ...
    for task in tasks_with_date:
        ...
        # El capitalize() asegura que el día de la semana inicie con mayúscula gracias al locale.
    ...
    for date_str, tasks in tasks_by_date.items():
        # Usar un expander para agrupar las tareas por día (estilo calendario)
        # Determinar el color de borde/énfasis por la tarea más urgente del día
        ...
        with st.expander(f"**{date_str}** ({len(tasks)} Tareas)", expanded=True):
            ...
            for task in tasks:
                ...
                # Diseño de tarjeta para cada tarea en el calendario
                # Se cambian los nombres de las columnas para reflejar el nuevo propósito
                ...
                with col_complete:
                    if st.button("✅", key=f"complete_{task['id']}", help="Marcar como realizada (Completar)", type="secondary"):
                        remove_task(task['id'])

                with col_btn_del:
                    if st.button("🗑️", key=f"del_date_{task['id']}", help="Eliminar tarea"):
                        remove_task(task['id'])